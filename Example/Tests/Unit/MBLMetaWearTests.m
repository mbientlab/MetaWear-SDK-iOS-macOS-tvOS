...
@interface MBLMetaWearTests : MBLMockMetaWearUnitTest
@end

@implementation MBLMetaWearTests

- (void)testRSSI
{
    XCTestExpectation *waitingExpectation = [self expectationWithDescription:@"wait for read"];

    [[self.device readRSSIAsync] success:^(NSNumber * _Nonnull result) {
        XCTAssertEqualWithAccuracy(result.doubleValue, -45.0, 0.1);
