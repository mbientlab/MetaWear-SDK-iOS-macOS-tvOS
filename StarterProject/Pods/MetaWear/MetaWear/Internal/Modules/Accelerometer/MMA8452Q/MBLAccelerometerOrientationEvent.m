/**
 * MBLAccelerometerOrientationEvent.m
 * MetaWear
 *
...
@interface MBLAccelerometerOrientationEvent ()
@property (nonatomic) MBLRegister *orientationEnable;
@property (nonatomic) MBLRegister *orientationSettings;
@end

@implementation MBLAccelerometerOrientationEvent
...
- (BFTask *)performAsyncActivation
{
    ...
    mma8452q_pl_regs_t regs = { 0 };
    regs.pl_cfg.dbcntm = YES;
    regs.pl_cfg.pl_en = YES;
    regs.pl_count.dbnce = [accelerometer scalePortraitLandscapeDebounceCounter:50.0];

    if (strobeGlobalEnable) {
        [tasks addObject:[accelerometer.globalEnable writeByteAsync:0x0]];
    }
    [tasks addObject:[self.orientationSettings writeDataAsync:[NSData dataWithBytes:&regs length:sizeof(mma8452q_pl_regs_t)]]];
    [tasks addObject:[self.orientationEnable writeByteAsync:0x1]];
    if (strobeGlobalEnable) {
...
{
    return [self.orientationEnable writeByteAsync:0x0];
}
