/**
 * MBLGPIOPinChangeEvent.m
 * MetaWear
 *
 * Created by Stephen Schiffli on 10/30/14.
...
@interface MBLGPIOPinChangeEvent ()
@property (nonatomic, weak) MBLGPIO *gpio;
@end

@implementation MBLGPIOPinChangeEvent

- (instancetype)initWithModule:(MBLGPIO *)gpio pin:(uint8_t)pin
{
...
{
    uint8_t data[] = { self.index, self.changeType };
    return [self.gpio.pinChangeSettings writeDataAsync:[NSData dataWithBytes:data length:2]];
}

- (BFTask *)performAsyncDeactivation
{
    uint8_t data[] = { self.index, 0 };
    return [self.gpio.pinChangeSettings writeDataAsync:[NSData dataWithBytes:data length:2]];
}

- (BFTask *)performAsyncStartNotifications
{
    return [[self.module.device.gpio startNotificationsOnChangeEvent:self] continueOnMetaWearWithSuccessBlock:^id _Nullable(BFTask * _Nonnull task) {
