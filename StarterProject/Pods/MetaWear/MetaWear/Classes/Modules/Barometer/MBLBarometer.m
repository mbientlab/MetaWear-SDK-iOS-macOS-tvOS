...
@property (nonatomic) MBLData *pressure;
@property (nonatomic) MBLData *altitude;
@end

@implementation MBLBarometer

+ (instancetype)objectWithDevice:(MBLMetaWear *)device moduleInfo:(MBLModuleInfo *)moduleInfo
{
    if (moduleInfo.moduleImplementation == 1) {
        return [[MBLBarometerBME280 alloc] initWithDevice:device moduleInfo:moduleInfo];
    }
    return [[MBLBarometerBMP280 alloc] initWithDevice:device moduleInfo:moduleInfo];
}
