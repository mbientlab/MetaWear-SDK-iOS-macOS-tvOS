...
typedef struct __attribute__((packed)) {
    uint8_t			filter_id;
    uint8_t			look_ahead;
    uint8_t			looking_for_max;
    uint8_t			extra_peak_width;
    uint32_t		delta;
    uint8_t			offset;
    uint8_t			length;
    uint8_t			is_signed;
} peak_detector_param_t;
...
{
    // TODO: This isn't ready, so its hidden...it might just die
    peak_detector_param_t params = {0};
    params.filter_id = 5;
    params.look_ahead = lookahead;
    params.looking_for_max = 1;
    params.extra_peak_width = extraPeakWidth;
    params.delta = delta;
    params.offset = self.format.offset;
    params.length = self.format.length;
    params.is_signed = isSigned;
    
    MBLFilter *filter = [[MBLFilter alloc] initWithTrigger:self
                                          filterParameters:[NSData dataWithBytes:&params length:sizeof(peak_detector_param_t)]
                                                    format:[[MBLNumericFormatter alloc] initIntWithLength:4 isSigned:isSigned]];
    return filter;
}
...
- (MBLEvent *)minPeakDetectorOfEventWithLookahead:(uint8_t)lookahead delta:(uint32_t)delta extraPeakWidth:(uint8_t)extraPeakWidth isSigned:(BOOL)isSigned
{
    ...
    MBLFilter *filter = [[MBLFilter alloc] initWithTrigger:self
                                          filterParameters:[NSData dataWithBytes:&params length:sizeof(peak_detector_param_t)]
                                              outputLength:4
                                                 formatter:isSigned ? [MBLNumericData signedFormatter] : [MBLNumericData unsignedFormatter]];
    return filter;
     */
    [NSException raise:@"Bad" format:@"This is not ready yet"];
    return nil;
}
...
typedef struct __attribute__((packed)) {
    ...
    uint8_t			look_ahead;
    int16_t			delta;
    uint16_t		punch_area_threshold;
    uint16_t		punch_start_threshold;
} punch_detector_param_t;
...
{
    // TODO: Hard coded to accelerometer X-axis
    punch_detector_param_t params = {0};
    params.filter_id = 4;
    params.offset = 0;
    params.length = 2;
    params.look_ahead = lookahead;
    params.delta = delta;
    params.punch_area_threshold = areaThreshold;
    params.punch_start_threshold = startThreshold;
    
    MBLFilter *filter = [[MBLFilter alloc] initWithTrigger:self
                                          filterParameters:[NSData dataWithBytes:&params length:sizeof(punch_detector_param_t)]
                                                    format:[[MBLNumericFormatter alloc] initIntWithLength:4 isSigned:NO]];
    return filter;
}
...
typedef struct __attribute__((packed)) {
    ...
    uint8_t      accel_shift;
    uint8_t      gyro_shift;
} fall_detector_param_t;

- (MBLFilter *)fallDetectorOfEventWithAccel:(uint8_t)accelShift gyro:(uint8_t)gyroShift
{
    fall_detector_param_t params = {0};
    params.filter_id = 16;
    params.accel_shift = accelShift;
    params.gyro_shift = gyroShift;
    
    MBLFilter *filter = [[MBLFilter alloc] initWithTrigger:self
                                          filterParameters:[NSData dataWithBytes:&params length:sizeof(fall_detector_param_t)]
                                                    format:[[MBLNumericFormatter alloc] initIntWithLength:4 isSigned:YES]];
                                                    //format:[[MBLNumericFormatter alloc] initDoubleWithLength:4 isSigned:YES scale:1.0 / 65536.0]];
