...
- (BFTask *)success:(MBLSuccessBlock)block
{
    return [self continueOnDispatchWithSuccessBlock:^id _Nullable(BFTask * _Nonnull task) {
        ...
        // produce a result so we just fake a @YES for those
        block(task.result ? task.result : @YES);
        return task;
    }];
}
...
void MBLForceLoadCategory_BFTask_MBLExtensions()
{
    NSString *string = nil;
    [string description];
}
